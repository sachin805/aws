...
public class OneMinRobo extends TimerTask {
	...
	public void run() {
		if(flag && timeCheck()) {
			...
			//	if (twelveMinCheck()) {
				...
				if ((array != null) && (length == 7) && (array[0].getLtp() != 0.0) && (array[6].getLtp() != 0.0)) {
				...
				boolean excludeFlag = excludeCheck((int)array[6].getLtp());
				...
				if ((upFirstMin && positionCheck) || (upFirstMinFourPoints && positionCheck)) {
					...
					ROBO_LOGGER.info("seven : " + seven + " one : " + one);
					ROBO_LOGGER.info("(seven-one) : " +(seven-one));
				...
				//	String[] reply =  RoboOrderExecutor.longFut();
					...
					RoboStore.twoMins = now.plusSeconds(120l);
				}
